<template>
  <div
    class="absolute bottom-6 left-[50%] transform -translate-x-[50%] z-[100] w-[94vw]"
  >
    <Button
  ...
  >
    <div v-if="product">
      ...
      <div>
        <ion-text class="font-bold mt-4 line-clamp-1">{{
          product.name
        }}</ion-text>
        <!-- Product rating -->
        <ul class="flex">
        ...
        <div class="flex justify-between py-4">
          ...
          <div class="flex items-center gap-4">
            <ion-button
              @click="updateQuantity('mainItem', -1)"
              color="tertiary"
              size="small"
            >
      ...
      <!-- Beverages section -->
      <div>
        <ion-text class="font-bold text-xl mt-2 line-clamp-1"
          >Beverages</ion-text
        >
        <div class="bg-neutral-100 rounded-2xl">
          <ion-select
            v-model="order.beverage.product"
            class="py-2 px-4"
            color="tertiary"
            label-placement="stacked"
            label="Choose Beverages"
          >
            <ion-select-option v-for="item in BEVERAGES" :value="item.id">{{
              item.name
            }}</ion-select-option>
          </ion-select>
        </div>
        <div class="flex">
          <div class="py-2 flex overflow-scroll">
            <Button
              @click="handleBeverageSize('')"
              :color="activeFilter == 'Regular' ? 'primary' : 'tertiary'"
              text="Regular"
              size="small"
            />
            <Button
              @click="handleBeverageSize('Large')"
              :color="activeFilter == 'Large' ? 'primary' : 'tertiary'"
              text="Large"
              size="small"
            />
            <Button
              @click="handleBeverageSize('X-Large')"
              :color="activeFilter == 'X-Large' ? 'primary' : 'tertiary'"
              text="X-Large"
              size="small"
            />
          </div>
      ...
      <div class="space-y-2 mb-20">
        ...
        <div
          v-for="(item, key) in addOnsAndRice"
          :key="key"
        ...
        >
          <div class="ion-padding bg-neutral-100 rounded-2xl flex">
            ...
            <div class="p-2 flex flex-col justify-between">
              ...
              <ion-text color="primary" class="font-bold text-xl"
                >P 150</ion-text
              >
            </div>
            <div class="flex items-center gap-4">
              <ion-button
                @click="updateQuantity(key, -1)"
                color="tertiary"
                size="small"
              >
                <ion-text color="primary">-</ion-text>
...
import { PRODUCTS, ADDONS, BEVERAGES } from "@/helpers/index.js";
import {
  IonText,
  IonIcon,
  IonTitle,
