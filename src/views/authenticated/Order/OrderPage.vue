  ...
  >
    <div class="space-y-2">
      <div class="flex items-center justify-center">
        <div
          class="w-full ion-padding bg-neutral-100 rounded-2xl flex flex-col"
        >
    ...
    <div class="mt-8">
      ...
      <div class="flex items-center justify-center">
        <div
          class="w-full ion-padding bg-neutral-100 rounded-2xl flex flex-col gap-4"
        >
          <div v-if="order" class="flex gap-4">
            <ion-img :src="order.img" class="w-20 h-20 aspect-square" />
            <div class="flex flex-col">
              <ion-label class="font-bold mb-2">{{
                order.mainItem.product.name
              }}</ion-label>
              <ion-text color="medium"
                >{{ order.sauce.quantity }}x {{ order.sauce.name }}</ion-text
              >
              <ion-text color="medium"
                >{{ order.rice.quantity }}x {{ order.rice.name }}</ion-text
              >
    ...
    <div
      class="mt-8 w-full ion-padding bg-neutral-100 rounded-2xl flex flex-col gap-4"
    >
      <div class="w-full flex flex-col">
