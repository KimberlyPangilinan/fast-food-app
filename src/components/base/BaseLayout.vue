<template>
  ...
  <ion-menu content-id="main-content">
    ...
    <ion-content class="ion-padding">
      <div class="flex flex-col mb-4">
        ...
        <ion-list lines="none">
          <ion-item
            v-for="item in MENU.slice(0, 5)"
            @click="navigate(`${item.to}`)"
            class="ion-no-padding py-1 items-center hover:!bg-red-50"
          >
            <ion-icon :icon="item.icon" color="primary"></ion-icon>
            <ion-text class="ion-padding-start font-bold">{{
              item.name
            }}</ion-text>
          </ion-item>
        </ion-list>
  ...
  <ion-page id="main-content">
    <div class="shadow-none p-4 flex justify-between items-center">
          ...
          <ion-menu-toggle v-else>
            <div class="flex flex-col gap-1">
              <div
                v-for="i in 2"
                :key="i"
                class="bg-red-600 w-6 rounded-full h-1"
              ></div>
            </div>
          </ion-menu-toggle>
      ...
      <div
        v-show="props.type == 'default' || props.type == 'order'"
...
import { MENU } from "@/helpers/index.js";
